...
public class SlcController implements SlcApi {
  ...
  @Override
  @GetMapping
  public ResponseEntity<SlcListaResponse> findAll() {
    logger.info("GET /slcs");
    final List<Slc> slcs = slcService.findAll();
    logger.info("GET /slcs - success.");

    return ResponseEntity.status(HttpStatus.OK)
        .body(new SlcListaResponse(slcs));
  }

  @Override
  @GetMapping("/{codMsg}")
  public ResponseEntity<Slc> findByCodMsg(@PathVariable String codMsg) {
    logger.info("GET /slcs/{}", codMsg);
    final Slc slc = slcService.findByCodMsg(codMsg);
    logger.info("GET /slcs/{} - success.", codMsg);
